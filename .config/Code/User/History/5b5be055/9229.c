...
static void win_loose_check(char **rays, char *sinput)
{
    ...
    double time_spent;
    unsigned int i = 0;
    dprintf(2, "ENTERED WIN/LOOSE\n");
    begin = clock();
    time_spent = (double)begin / CLOCKS_PER_SEC;
    dprintf(2, "\nrays[16] == %f\nrays[0] == %f\nrays[31] == %f\n", atof(rays[16]), atof(rays[0]), atof(rays[31]));
    if (atof(rays[16]) <= 150 || atof(rays[0]) <= 150 || atof(rays[31]) <= 150) {

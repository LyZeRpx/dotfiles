...
import_lib() {
    ...
    echo List of Avaliable Libraries : ; ls --color=auto ~/lib_workshop/
    read lib_name
    cp -r ~/lib_workshop/lib$lib_name ./libraries/lib$lib_name 2> /dev/null
    if [ "$?" == 1 ]; then
        ...
        if [ "$input" == "$YES" ]; then
            ...
            read lib_name
            cp -r ~/lib_workshop/lib$lib_name ./libraries/$lib_name 2> /dev/null
            if [ "$?" == 1 ]; then
